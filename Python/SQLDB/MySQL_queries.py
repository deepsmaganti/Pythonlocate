...
class DB_Queries():
    ...
    def where_str(self, cond, ao):
        ...
        for i in (range(len(cond.keys()) - 1)):
            ...
            if (ao == None):
                if type(cond.values()[i]) == int:
                    string = string + cond.keys()[i] + " = " + str(cond.values()[i])
                elif type(cond.values()[i]) == str:
                    string = string + cond.keys()[i] + " = '" + str(cond.values()[i])
    ...
    def delete(self, tablename, cond):
        mycursor = self.conn.cursor()
        try:
            query = "delete from "+tablename+" where "+self.where_str(cond, None)+";"
            logging.info(' SQL Query Executed: ' + query)
            mycursor.execute(query)
            self.conn.commit()
        except mysql.connector.Error as err:
...
#xyz.create('new_table','id int not null, name varchar(20), title varchar(10), primary key (id)')
#xyz.insert('new_table', {'id' : 0, 'name' : 'Dipti', 'title' : 'Engineer'})
#xyz.insert('new_table', {'id' : 1, 'name' : 'Dipti', 'title' : 'Engineer'})
#xyz.delete('new_table',{'id' : 1})